...
Marketplace 등록이 완료된 경우 Claude Code 안에서 다음 순서로 설치합니다.

```text
/plugin marketplace add MTGVim/tigap-skills
/plugin install tigap-skills@MTGVim/tigap-skills
/reload-plugins
```
...
> Marketplace에 게시하려면 플러그인 매니페스트(`.claude-plugin/plugin.json`)와 marketplace 설정이 필요합니다.
...
Standalone 모드에서는 다음처럼 호출할 수 있습니다.
...
기본적으로 생성되는 작업 산출물은 현재 프로젝트 아래에 저장합니다.
...
- 분석 결과와 구현 계획은 프로젝트 로컬 산출물로 저장합니다.
